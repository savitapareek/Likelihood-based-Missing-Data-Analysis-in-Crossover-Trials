...
sim1=read.csv(file.choose(),header = T) ##read allest_100_.csv
head(sim1)
View(sim1)
## store the results; one for 100 simulations and other 500 simulations
sim100=sim1[1:100,c(2:11)]
...
trep=nrow(esti)
obs=matrix(nrow = 10,ncol=nrow(esti))
for (i in 1:10) {
...
df33 <-data.frame(parameter=rep(c("Intercept","Period1","Period2","Trt1","Trt2","Res1","Res2","Res3","sigma_e^2","sigma_s^2"), each=trep),
                 parameter_value=c(x,x2,x3,x4,x5,x6,x7,x8,x9,x10),
                 relative_frequency=c(y,y2,y3,y4,y5,y6,y7,y8,y9,y10))
