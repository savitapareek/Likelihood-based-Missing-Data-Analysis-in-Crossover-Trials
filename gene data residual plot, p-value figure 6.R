...
plot(estimatePDF(unlist(re)))
skewness(unlist(re))
qqnorm(unlist(re))
qqline(unlist(re))
standardised_residuals=scale(residu)[-382]
df=data.frame(standardised_residuals)
p1=ggplot(df, aes(standardised_residuals)) + geom_density()+       # Draw histogram with density
  stat_function(fun = dnorm,
                args = list(mean = mean(standardised_residuals),
                            sd = sd(standardised_residuals)),
                col = "blue",
                size = 1)+theme_classic()
p12=p1+ylab("Density")+xlab("Standardized residuals")+xlim(-4,6)+ylim(0,1)+
  theme(axis.title=element_text(size=10))
pp <- ggplot(df, aes(sample =standardised_residuals))+ stat_qq() + stat_qq_line()+theme_classic()
p2=pp+xlab("Theoretical Quantiles")+ylab("Sample Quantiles")+
  theme(axis.title=element_text(size=10))

figure1 <- ggarrange(p12, p2,
                     
                     ncol = 2, nrow = 1,labels = c("A","B"),
                     font.label=list(color="black",size=9))
figure1
 ggarrange(figure1,figure2,nrow=2,ncol=1)
# ggarrange(p11+ylim(0,1)+xlab("Standardized residuals \n (original model)"),p12,nrow=1,ncol=2,labels = c("A","B"))
