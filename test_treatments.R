...
cat("enter missing probabilities in each sequence, total seq=",s)
prob=c(.15,.15)
n=pmn=n1=list()
no=c(5,5) ## no of subjects in different sequences
for (i in 1:s) {
  n[[i]]=diag(no[i])
  pmn[[i]]=diag(no[i]*p*m)
  n1[[i]]=rep(1,no[i])
...
## X matrix under reduced model
xr=lapply(1:s, function(i) x[[i]][,-3])
...
festi=foreach(sim= 1:nrep,.combine = rbind,.packages = c('psych','parallel','MASS'))%dopar% {
  ...
  #estimation under H0
  beth=tbet[-c(3)]-.12 # make a note: under reduced model
  sigmah=tsigma-.1
  sigmash=tsigmas+.2
  ...
  cbind(estti,truest[-3]) #estimates under H0 when data is generated from full
  
  ## estimate under H1 when data is generated from model
  beth=tbet-.12 # make a note
  sigmah=tsigma-.1
  ...
  ##under H0
  beth=estti[1:(p+t+m-2-3+2)];sigmae=estti[p+t+m-1-3+2];sigmash=estti[p+t+m-3+2]
  iD=mclapply(n,"*",sigmash)
  sig0i=mclapply(mcMap("+",mclapply(mclapply(z, crossprod), "*",sigmae**-1),mclapply(iD, solve)),solve)
  mumis=mclapply(misx1, "%*%",beth)
  muobs=mclapply(obsx1, "%*%",beth)
...
stopCluster(cl1)
powr=as.numeric((colSums(festi)[(p+t+m)+1])/nrep) # writing power
powr
Sys.time()-strt